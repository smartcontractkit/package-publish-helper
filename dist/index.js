...
const REMOTE_VERSION_MAX_TRIES = 5;
const REMOTE_VERSION_FAIL_TIMEOUT = 3000;
...
function getRemoteVersion(name, count = 0) {
...
const validateEnvironment = (mode, folder, folders, branch) => {
    ...
    let failed = false;
    if (!GITHUB_WORKSPACE) {
        failedMsg.push(`GITHUB_WORKSPACE not set in environment. Did you check out the repository?`);
        failed = true;
    }
    if (mode === 'publish') {
        if (!folder) {
...
var modes_awaiter = (undefined && undefined.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
function shouldPackagePublish(folder) {
    return modes_awaiter(this, void 0, void 0, function* () {
        const output = [];
        let should_publish;
        ...
        if (semver_default().gt(version, remoteVersion)) {
            console.log(Object(source.green)(`Package should publish`));
            should_publish = 'yes';
        }
        else {
            should_publish = 'no';
...
function shouldVersionBeUpdated(foldersStr, targetBranch) {
    return modes_awaiter(this, void 0, void 0, function* () {
        ...
        for (const folder of folders) {
            ...
            if (semver_default().lte(localPackage.version, targetPackage.version)) {
                ...
                console.log(Object(source.green)(`Changes found for ${folder} but version has been updated`));
            }
        }
        return { output, error };
    });
...
function main() {
    return src_awaiter(this, void 0, void 0, function* () {
        const mode = Object(core.getInput)('mode');
        const folders = Object(core.getInput)('folders');
        const folder = Object(core.getInput)('folder');
        const branch = Object(core.getInput)('branch');
        const { failed, failedMsg } = validateEnvironment(mode, folder, folders, branch);
        if (failed) {
        ...
        try {
            if (mode === 'publish') {
                ...
                Object(core.setOutput)('should_publish', should_publish);
            }
            else {
                const { error, output } = yield shouldVersionBeUpdated(folders, branch);
                if (error) {
                    Object(core.setFailed)(output.join('\n'));
                }
            }
        }
        catch (e) {
